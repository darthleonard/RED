...
public class Tool {
	...
	private String red;
	private String redb;
	private final DeviceDataset datos;
	...
	private DeviceRecord datosLocales() throws SocketException {
		boolean flag = true;
		...
		DeviceRecord localDevice = new DeviceRecord();
		localDevice.setIpAddress(inet.getHostAddress());
		localDevice.setMacAddress(obtenerMacLocal(inet));
		localDevice.setDescription("Equipo local");
		return localDevice;
	}
	...
	public void LimpiaArp() throws IOException {
		...
		if (so.equals("Linux")) {
			...
			// comando = new String("cmd /c runas /user:altermannrt@hotmail.com \"netsh
			// interface ip delete arpcache\"");
			comando = new String("cmd /c start test.bat");
		}

		// String[] c = {"runas", "/user:altermannrt@hotmail.com", "\"netsh interface ip
		// delete arpcache\""};
	...
	private void onLinux(BufferedReader br) throws IOException {
		...
		while (br.ready()) {
			...
			if (!b.contains("(")) {
				...
				device.setIpAddress(a);
				device.setMacAddress(b.toUpperCase());
